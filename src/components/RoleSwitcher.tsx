...
import { ShieldCheck, Building2, HardHat, Sparkles, Briefcase, Lock, Unlock, LogOut, LayoutDashboard, UserCircle, Globe, HeartHandshake } from "lucide-react";

type AppRole = 'admin' | 'company' | 'tech' | 'comercial' | 'manager' | 'customer' | 'distributor' | 'partner';
...
export default function RoleSwitcher({ 
  ...
  const currentUser = authenticatedUsers[currentRole as string];
  const hasToken = !!tokens[currentRole as string];
    ...
    <div className="bg-[#080a10]/95 backdrop-blur-md border-b border-slate-800/60 py-3.5 px-6 text-white sticky top-0 z-50">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          ) : (['manager', 'customer', 'distributor', 'partner'].includes(currentRole)) ? (
            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl text-xs text-emerald-400 font-mono">
              <Unlock className="h-3.5 w-3.5" />
              <span>Portal de demonstração ativo</span>
            </div>
          ) : null}
        </div>
