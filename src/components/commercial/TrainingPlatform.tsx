      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Simulações Práticas</h3>
              <p className="text-[10px] text-slate-500">Ambiente seguro para praticar fluxos reais</p>
            </div>
          </div>
          <div className="space-y-2">
            {[
              { title: "Simular abertura de chamado", difficulty: "Fácil", done: true, icon: "🎫" },
              { title: "Despachar técnico manualmente", difficulty: "Fácil", done: true, icon: "📍" },
              { title: "Aprovar fechamento de chamado", difficulty: "Médio", done: false, icon: "✅" },
              { title: "Gerar relatório com IA", difficulty: "Médio", done: false, icon: "🤖" },
              { title: "Configurar split de pagamento", difficulty: "Difícil", done: false, icon: "💳" },
              { title: "Responder a alerta de fraude", difficulty: "Difícil", done: false, icon: "🛡️" },
            ].map((sim, i) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${
                ...
                <span className="text-lg shrink-0">{sim.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-white truncate">{sim.title}</div>
                  <div className={`text-[9px] font-mono mt-0.5 ${
                    sim.difficulty === "Fácil" ? "text-emerald-400" :
                    sim.difficulty === "Médio" ? "text-yellow-400" : "text-red-400"
                  }`}>{sim.difficulty}</div>
                </div>
                {sim.done
                  ? <CheckCircle className="h-4 w-4 text-emerald-400 shrink-0" />
                  : <button className="text-[9px] bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded-lg font-semibold hover:bg-cyan-500/30 transition-all">Iniciar</button>
                }
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white">Avaliações e Quizzes</h3>
              <p className="text-[10px] text-slate-500">Valide seu conhecimento e ganhe certificados</p>
            </div>
          </div>
          <div className="space-y-2">
            {[
              { title: "Quiz: Conceitos Básicos FSM", questions: 10, score: 90, done: true, minScore: 70 },
              { title: "Quiz: Despacho e IA", questions: 15, score: 80, done: true, minScore: 70 },
              { title: "Quiz: Financeiro e Pagamentos", questions: 12, score: null, done: false, minScore: 80 },
              { title: "Quiz: Segurança e LGPD", questions: 8, score: null, done: false, minScore: 75 },
              { title: "Avaliação Final — Administrador", questions: 25, score: null, done: false, minScore: 85 },
            ].map((q, i) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                q.done
                  ? (q.score! >= q.minScore ? "border-emerald-500/20 bg-emerald-500/5" : "border-red-500/20 bg-red-500/5")
                  : "border-slate-800 hover:border-indigo-500/30 cursor-pointer hover:bg-indigo-500/5"
              }`}>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold text-white">{q.title}</div>
                  <div className="text-[9px] text-slate-500 mt-0.5">{q.questions} questões · Mínimo {q.minScore}%</div>
                </div>
                {q.done ? (
                  <div className="text-right shrink-0">
                    <div className={`text-sm font-bold ${q.score! >= q.minScore ? "text-emerald-400" : "text-red-400"}`}>{q.score}%</div>
                    <div className="text-[9px] text-slate-500">{q.score! >= q.minScore ? "Aprovado" : "Refazer"}</div>
                  </div>
                ) : (
                  <button className="text-[9px] bg-indigo-500/20 text-indigo-400 px-2 py-1 rounded-lg font-semibold hover:bg-indigo-500/30 transition-all shrink-0">
                    Fazer Quiz
                  </button>
                )}
              </div>
          ...
          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-3 flex items-center gap-3">
            ...
            <div className="text-xs text-slate-400">
              Complete todos os quizzes com nota ≥ mínimo para desbloquear o <strong className="text-white">Certificado de Proficiência NexoraField</strong>.
            </div>
          </div>
        </div>
      </div>
