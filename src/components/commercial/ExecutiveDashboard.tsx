...
// Add Time to Value metric to CEO section after other bottom metrics
...
export default function ExecutiveDashboard({ companies, technicians, tickets, transactions, auditLogs }: ExecutiveDashboardProps) {
  const [activeTab, setActiveTab] = useState<"ceo" | "commercial" | "cs" | "financial" | "revops" | "marketing" | "partners">("ceo");
    ...
    <div className="space-y-6">
      ...
      {activeTab === "ceo" && (
        <div className="space-y-6">
          ...
          {/* Time to Value row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard title="Time to Value" value="4.2 dias" sub="Do onboarding ao 1º chamado" trend="-1.3d" trendUp icon={Zap} color="emerald" />
            <KPICard title="Onboarding Completo" value="89%" sub="Taxa de conclusão 30d" trend="+6%" trendUp icon={Activity} color="cyan" />
            <KPICard title="Feature Adoption" value="67%" sub="Módulos usados/disponíveis" trend="+11%" trendUp icon={BarChart2} color="indigo" />
            <KPICard title="Expansão MRR" value="+R$ 8.2k" sub="Upgrades e add-ons no mês" trend="+22%" trendUp icon={TrendingUp} color="yellow" />
          </div>
      ...
      {activeTab === "marketing" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard title="Visitantes/Mês" value="8.400" sub="Tráfego orgânico" trend="+22%" trendUp icon={Globe} color="purple" />
            <KPICard title="Leads Gerados" value="1.240" sub="MQLs qualificados" trend="+18%" trendUp icon={Users} color="indigo" />
            <KPICard title="Custo por Lead" value="R$ 28" sub="CPL médio" trend="-12%" trendUp icon={Target} color="emerald" />
            <KPICard title="Taxa de Conversão" value="4.8%" sub="Visitante → Trial" trend="+0.6%" trendUp icon={TrendingUp} color="cyan" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5">
              <h3 className="text-sm font-bold text-white mb-4">Canais de Aquisição</h3>
              <div className="space-y-3">
                {[
                  { label: "Orgânico (SEO)", pct: 38, color: "bg-emerald-500", leads: 471 },
                  { label: "Google Ads", pct: 24, color: "bg-blue-500", leads: 298 },
                  { label: "Indicação", pct: 19, color: "bg-cyan-500", leads: 236 },
                  { label: "LinkedIn", pct: 11, color: "bg-indigo-500", leads: 136 },
                  { label: "Direto", pct: 8, color: "bg-slate-500", leads: 99 },
                ].map(row => (
                  <div key={row.label} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-mono text-slate-300 w-8 text-right">{row.leads}</span>
                    <span className="text-[10px] text-slate-600 w-8">{row.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5">
              <h3 className="text-sm font-bold text-white mb-4">Funil de Marketing</h3>
              <div className="space-y-2">
                {[
                  { label: "Visitantes únicos", value: 8400, pct: 100, color: "bg-slate-600" },
                  { label: "Leads (e-mail capturado)", value: 1240, pct: 14.8, color: "bg-indigo-500" },
                  { label: "MQL (score ≥ 40)", value: 420, pct: 5.0, color: "bg-purple-500" },
                  { label: "Trial iniciado", value: 64, pct: 0.8, color: "bg-cyan-500" },
                  { label: "Convertido", value: 31, pct: 0.4, color: "bg-emerald-500" },
                ].map(s => (
                  <div key={s.label} className="flex items-center gap-3">
                    <span className="text-[10px] text-slate-500 w-36 shrink-0">{s.label}</span>
                    <div className="flex-1 h-3 bg-slate-800 rounded-full overflow-hidden">
                      <div className={`h-full ${s.color} rounded-full`} style={{ width: `${s.pct}%` }} />
                    </div>
                    <span className="text-xs font-mono text-slate-300 w-12 text-right">{s.value.toLocaleString("pt-BR")}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard title="CAC Marketing" value="R$ 280" sub="Custo por cliente" trend="-R$ 40" trendUp icon={Target} color="yellow" />
            <KPICard title="ROAS" value="8.4x" sub="Retorno sobre ads" trend="+1.2x" trendUp icon={TrendingUp} color="emerald" />
            <KPICard title="NPS Captação" value="68" sub="Qualidade dos leads" trend="+5" trendUp icon={Star} color="cyan" />
            <KPICard title="Churn Trial" value="74%" sub="Trial → abandono" trend="-3%" trendUp icon={Activity} color="red" />
          </div>
        </div>
      )}
      ...
      {activeTab === "partners" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard title="Parceiros Ativos" value="28" sub="Indicadores e distribuidores" trend="+6" trendUp icon={Users} color="yellow" />
            <KPICard title="Receita Parceiros" value="R$ 22k" sub="MRR via canal" trend="+34%" trendUp icon={DollarSign} color="emerald" />
            <KPICard title="Indicações do Mês" value="47" sub="Novos leads via parceiros" trend="+12" trendUp icon={TrendingUp} color="cyan" />
            <KPICard title="Conversão Parceiros" value="36%" sub="Indicação → Cliente" trend="+8%" trendUp icon={Target} color="indigo" />
          </div>
          <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-white mb-4">Performance por Tipo de Parceiro</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { type: "Indicadores", count: 18, mrr: "R$ 8.4k", conv: "28%", color: "yellow" },
                { type: "Distribuidores", count: 8, mrr: "R$ 10.2k", conv: "42%", color: "orange" },
                { type: "White Label", count: 2, mrr: "R$ 3.4k", conv: "100%", color: "indigo" },
              ].map((tier, i) => (
                <div key={i} className={`bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3`}>
                  <div className={`text-sm font-bold text-${tier.color}-400`}>{tier.type}</div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <div className="text-lg font-bold text-white">{tier.count}</div>
                      <div className="text-[9px] text-slate-500">Parceiros</div>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-emerald-400">{tier.mrr}</div>
                      <div className="text-[9px] text-slate-500">MRR</div>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-cyan-400">{tier.conv}</div>
                      <div className="text-[9px] text-slate-500">Conversão</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-5">
            <h3 className="text-sm font-bold text-white mb-4">Top 5 Parceiros — MRR Gerado</h3>
            <div className="space-y-3">
              {[
                { name: "Agência Digital SP", type: "Distribuidor", mrr: 4200, leads: 18 },
                { name: "TechSales Brasil", type: "Distribuidor", mrr: 3100, leads: 14 },
                { name: "CloudPartners Ltda", type: "Indicador", mrr: 2400, leads: 9 },
                { name: "NetConsult RJ", type: "Indicador", mrr: 1800, leads: 7 },
                { name: "FSM Solutions", type: "Distribuidor", mrr: 1600, leads: 6 },
              ].map((p, i) => (
                <div key={i} className="flex items-center gap-4 p-3 bg-slate-900/60 rounded-xl">
                  <span className="text-xs font-bold text-slate-500 w-4">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-semibold text-white">{p.name}</div>
                    <div className="text-[10px] text-slate-500">{p.type} · {p.leads} leads</div>
                  </div>
                  <span className="text-xs font-bold text-emerald-400">R$ {p.mrr.toLocaleString("pt-BR")}/mês</span>
                </div>
              ))}
            </div>
          </div>
