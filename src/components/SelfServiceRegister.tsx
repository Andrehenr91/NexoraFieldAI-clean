...
export default function SelfServiceRegister({ onSuccess, onBack }: SelfServiceRegisterProps) {
  ...
  const handleVerifySms = () => {
    // Accept "123456" as demo code, or any 6-digit code in demo mode
    if (smsCode.length === 6) {
      setSmsVerified(true);
      setErrors({});
    } else {
      setErrors({ sms: "Código inválido. Use 6 dígitos (ex: 123456 para demo)" });
    }
  };
      ...
      <div className="w-full max-w-2xl bg-[#0b0e17] border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
        ...
        {step === 3 && (
          <div className="p-8 space-y-6">
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">Enviaremos um código para {phone || "(11) 9xxxx-xxxx"}</div>
                </div>
              </div>
              {!smsSent ? (
                ...
                <div className="space-y-3">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 text-xs text-emerald-400 text-center">
                    ✓ SMS enviado para {phone || "(11) 9xxxx-xxxx"} · Use <strong>qualquer código de 6 dígitos</strong> no modo demo
                  </div>
                  {smsVerified ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={handleVerifySms}
                          className="px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition-all">
                          Verificar
                        </button>
                      </div>
                      {errors.sms && <p className="text-red-400 text-[10px]">{errors.sms}</p>}
                      <button onClick={handleSendSms} className="text-xs text-slate-500 hover:text-slate-300 transition-all">
                        Reenviar código
                      </button>
                    </div>
                  )}
            ...
            {errors.sms && !smsSent && <p className="text-red-400 text-xs">{errors.sms}</p>}
          </div>
        )}
        ...
        {step < 6 && (
          <div className="px-8 pb-8 flex gap-3">
            ...
            <button
              ...
              {step === 5 ? "Confirmar e Criar Conta" : step === 3 && !smsVerified ? "Já verifiquei meu número" : "Continuar"}
              {step < 5 && !loading && <ArrowRight className="h-4 w-4" />}
            </button>
          </div>
        )}
      </div>
